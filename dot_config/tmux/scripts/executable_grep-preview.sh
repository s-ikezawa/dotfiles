...
# Get matching line numbers (mapfile is bash 4+; use a read loop for macOS bash 3.2)
lines=()
while IFS= read -r l; do
  [[ -n $l ]] && lines+=("$l")
done < <(rg --line-number --no-heading --color=never -I -- "$query" "$file" 2>/dev/null | cut -d: -f1 | head -20)
