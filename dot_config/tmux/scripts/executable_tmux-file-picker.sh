...
# Directory of this script (used to locate sibling helpers like grep-preview.sh)
...
main() {
  ...
  if $select_directories; then
    ...
    grep_toggle_flags=(
      ...
      --preview 'if [[ $FZF_PROMPT == Grep* ]]; then '"$SCRIPT_DIR"'/grep-preview.sh {q} {}; else '"$preview_cmd"'; fi'
