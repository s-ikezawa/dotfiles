...
**リポジトリの履歴に別の型体系があれば、そちらが優先。** 以下は履歴から規約を読み取れない
ときの既定（Conventional Commits）。
...
| `fix` | バグ修正 |
| `docs` | ドキュメントのみの変更 |
| `style` | 挙動を変えない整形（空白、セミコロン、フォーマッタ適用） |
...
フッタの `Key: Value` 行は `git interpret-trailers` でパースでき、`git log --grep` で検索
できる。コミットに機械可読なインデックスを付ける仕組みとして使う。
...
```text
feat(api)!: レスポンスの日付形式を ISO 8601 に変更

BREAKING CHANGE: /v1/events の created_at が Unix 秒から ISO 8601 文字列になる。
クライアントはパース処理の更新が必要。
```

`BREAKING CHANGE` は大文字必須（`BREAKING-CHANGE` も同義）。`!` とフッタは併用してよい。
