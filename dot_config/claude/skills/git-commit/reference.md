...
- **過去のコミットの記述を後から訂正するなら `Amends: <sha>` を足す**（別リポジトリなら
  `Amends: <repo>@<sha>`）。本文に書くだけでは、訂正された SHA を先に知っている人しか
  たどり着けない。トレーラなら `git log --grep='^Amends:'` で訂正の記録を型として引ける。
  **署名などと同じ最後の段落に置くこと。** git が解釈するのは最後の段落だけなので、
  間に空行を入れて別段落にすると `interpret-trailers --parse` から見えなくなり、
  ただの本文になる。`--trailer 'Amends: <sha>'` で足せば位置は自動的に正しくなる。
