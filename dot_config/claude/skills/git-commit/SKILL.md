...
（`--branch` を付けているので出力そのものは決して空にならない）。**`##` 行が 1 行も無いときは
`git status` 自体が失敗している** — 壊れた index、bare リポジトリ、`.git` 内での実行などで、
注入では stderr を捨てているため理由が出ない。`git status --short --branch` を実行し直して
エラーを読んでから判断する。「リポジトリではない」と決めつけない。
...
- [ ] 検証したことを書くなら、そのままコピーして再現できる（コマンド・対象パス・環境の
      バージョンまで書く。作業ツリーで変わる絶対値を再現手順にしない）
- [ ] 断定の根拠が、自分で見た一次情報か。他者の報告や推測を一次情報の顔で書いていないか
- [ ] その変更が壊しうる当のものを検証したか（構文や形式の検証だけで済ませていないか）
